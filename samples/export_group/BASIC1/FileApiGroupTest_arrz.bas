...
Rem Copyright (C) 2025-02-08 Kay Gürtzig 
...
Public Function generateRandomNumberFile(fileName As String, count As Integer, minVal As integer, maxVal As integer) As integer
  ...
  Try
    ...
    Next k
  Catch ex13efeb7d As Exception
    Dim error As String = ex13efeb7d.ToString()
    PRINT error
    Return -7
  Finally
    fileClose(fileNo)
...
If fileNo > 0 Then
  ...
  Catch ex5e8fc26c As Exception
    Dim failure As String = ex5e8fc26c.ToString()
    PRINT failure
  End Try
  If nObtained > 0 Then
    min = numberArray(0)
