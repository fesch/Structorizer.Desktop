...
Rem be sensible. 
Rem Generated by Structorizer 3.32-36 

Rem Copyright (C) 2026-05-02 Kay Gürtzig 
Rem License: GPLv3-Link 
Rem GNU General Public License (V 3)  
Rem https://www.gnu.org/licenses/gpl.html 
Rem https://www.gnu.de/documents/gpl-3.0.de.html  
...
Public Function generateRandomNumberFile(fileName As String, count As Integer, minVal As integer, maxVal As integer) As integer
  ...
  Try
    ...
    Next k
  Catch ex300c02e9 As Exception
    Dim error As String = ex300c02e9.ToString()
    PRINT error
    Return -7
  Finally
    fileClose(fileNo)
...
Public Function readNumbers(fileName As String, numbers As integer(50), maxNumbers As integer) As integer
  ...
  Try
    Do While NOT fileEOF(fileNo) AND nNumbers < maxNumbers
      ...
      nNumbers = nNumbers + 1
    Loop
  Catch ex779c0760 As Exception
    Dim error As String = ex779c0760.ToString()
    Throw 
  Finally
    fileClose(fileNo)
  End Try
...
Rem   fileClose(fileNo) 
  ...
  Catch ex3404182 As Exception
    Dim failure As String = ex3404182.ToString()
    PRINT failure
...
Rem Reads a random number file and draws a histogram according to the 
...
If fileNo > 0 Then
  ...
  Catch ex10b0be71 As Exception
    Dim failure As String = ex10b0be71.ToString()
    PRINT failure
  End Try
  If nObtained > 0 Then
    min = numberArray(0)
