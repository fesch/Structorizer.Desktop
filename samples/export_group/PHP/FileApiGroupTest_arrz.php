...
// Copyright (C) 2026-05-02 Kay Gürtzig 
// License: GPLv3-Link 
...
function generateRandomNumberFile($fileName, $count, $minVal, $maxVal)
{
	...
	} catch (Exception $ex300c02e9) {
		$error = $ex300c02e9->getMessage();
		echo error;
		return -7;
	} finally {
...
function readNumbers($fileName, $numbers, $maxNumbers)
{
	...
	} catch (Exception $ex779c0760) {
		$error = $ex779c0760->getMessage();
		throw $ex779c0760;
	} finally {
		StructorizerFileAPI::fileClose($fileNo);
	}
...
// 	StructorizerFileAPI::fileClose($fileNo); 
	...
	} catch (Exception $ex3404182) {
		$failure = $ex3404182->getMessage();
		echo failure;
		exit(-7);
	}
	$sum = 0.0;
...
if ($fileNo > 0)
{
	...
	} catch (Exception $ex10b0be71) {
		$failure = $ex10b0be71->getMessage();
		echo failure;
	}
	if ($nObtained > 0)
	{
