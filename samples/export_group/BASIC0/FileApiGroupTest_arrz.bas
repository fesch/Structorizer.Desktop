...
1770 REM Generated by Structorizer 3.32-36 
1780 
...
1830 REM https://www.gnu.de/documents/gpl-3.0.de.html 
1840 
1850 REM  
...
2070     PRINT failure
2080     REM FIXME: The following exit code was intended to pass: -7 
2090     STOP
2100   REM End of error handler, resume here ... 
2110   ON ERROR GOTO 0
2120   LET sum = 0.0
2130   FOR k = 0 TO nValues-1
2140     LET sum = sum + values(k)
2150   NEXT k
2160   PRINT "sum = "; sum
2170   PRINT "average = "; sum / nValues
2180 END IF
2190 END
2200 
2210 REM ======= 8< =========================================================== 
2220 
2230 REM Reads a random number file and draws a histogram accotrding to the 
2240 REM user specifications 
2250 REM Generated by Structorizer 3.32-36 
2260 
2270 REM Copyright (C) 2020-03-21 Kay Gürtzig 
2280 REM License: GPLv3-link 
2290 REM GNU General Public License (V 3) 
2300 REM https://www.gnu.org/licenses/gpl.html 
2310 REM https://www.gnu.de/documents/gpl-3.0.de.html 
2320 
2330 REM  
2340 REM program DrawRandomHistogram
2350 REM TODO: add the respective type suffixes to your variable names if required 
2360 DIM numberArray() AS ???
2370 DIM count() AS Integer
2380 REM  
2390 LET fileNo = -10
2400 DO
2410   PRINT "Name/path of the number file"; : INPUT file_name
2420   LET fileNo = fileOpen(file_name)
2430 LOOP UNTIL fileNo > 0 OR file_name = ""
2440 IF fileNo > 0 THEN
2450   fileClose(fileNo)
2460   PRINT "number of intervals"; : INPUT nIntervals
2470   REM Initialize the interval counters 
2480   FOR k = 0 TO nIntervals-1
2490     LET count(k) = 0
2500   NEXT k
2510   REM Index of the most populated interval 
2520   LET kMaxCount = 0
2530   LET nObtained = 0
2540   ON ERROR GOTO 2580
2550   LET nObtained = readNumbers(file_name, numberArray, 10000)
2560   GOTO 2600
2570   REM Start of error handler, FIXME: variable 'failure' should conatain error info ... 
2580     PRINT failure
2590   REM End of error handler, resume here ... 
2600   ON ERROR GOTO 0
2610   IF nObtained > 0 THEN
2620     LET min = numberArray(0)
2630     LET max = numberArray(0)
2640     FOR i = 1 TO nObtained-1
2650       IF numberArray(i) < min THEN
2660         LET min = numberArray(i)
2670       ELSE
2680         IF numberArray(i) > max THEN
2690           LET max = numberArray(i)
2700         END IF
2710       END IF
2720     NEXT i
2730     REM Interval width 
2740     LET width = (max - min) * 1.0 / nIntervals
2750     FOR i = 0 TO nObtained - 1
2760       LET value = numberArray(i)
2770       LET k = 1
2780       DO WHILE k < nIntervals AND value > min + k * width
2790         LET k = k + 1
2800       LOOP
2810       LET count(k-1) = count(k-1) + 1
2820       IF count(k-1) > count(kMaxCount) THEN
2830         LET kMaxCount = k-1
2840       END IF
2850     NEXT i
2860     CALL drawBarChart(count, nIntervals)
2870     PRINT "Interval with max count: "; kMaxCount; " ("; count(kMaxCount); ")"
2880     FOR k = 0 TO nIntervals-1
2890       PRINT count(k); " numbers in interval "; k; " ("; min + k * width; " ... "; min + (k+1) * width; ")"
2900     NEXT k
2910   ELSE
2920     PRINT "No numbers read."
2930   END IF
2940 END IF
2950 END
