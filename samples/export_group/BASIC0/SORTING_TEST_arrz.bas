...
1030         LET values(i) = values(i+1)
1040         LET values(i+1) = temp
1050         LET posSwapped = i
1060       END IF
1070     NEXT i
1080     LET ende = posSwapped - 1
...
1220   LET right = (i+1) * 2
1230   LET left = right - 1
...
1310   END IF
1320   IF max <> i THEN
1330     LET temp = heap(i)
1340     LET heap(i) = heap(max)
1350     LET heap(max) = temp
...
2180   REM  
2190   REM  
2200   LET lgth = length(heap)
2210   FOR k = lgth / 2 - 1 TO 0 STEP -1
...
2380     LET p = random(stop-start) + start
...
2410     LET p = partition(values, start, stop, p)
...
2750   LET heapRange = length(values)
2760   FOR k = heapRange - 1 TO 1 STEP -1
2770     LET heapRange = heapRange - 1
2780     REM Swap the maximum value (root of the heap) to the heap end 
2790     LET maximum = values(0)
2800     LET values(0) = values(heapRange)
2810     LET values(heapRange) = maximum
