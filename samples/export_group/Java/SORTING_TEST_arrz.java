...
public class SORTING_TEST_MAIN {
	...
	public static void main(String[] args) {
		...
		class Worker5e422485_0 implements Callable<Object[]> {
			private int[] values1;
			public Worker5e422485_0(int[] values1) {
		...
		class Worker5e422485_1 implements Callable<Object[]> {
			private ???[] values2;
			private ??? elementCount;
			public Worker5e422485_1(???[] values2, ??? elementCount) {
		...
		class Worker5e422485_2 implements Callable<Object[]> {
			private ???[] values3;
			public Worker5e422485_2(???[] values3) {
		...
		try {
			...
			// ----------------- START THREAD 0 ----------------- 
			Future<Object[]> future5e422485_0 = pool.submit( new Worker5e422485_0(values1) );
		
			// ----------------- START THREAD 1 ----------------- 
			Future<Object[]> future5e422485_1 = pool.submit( new Worker5e422485_1(values2, elementCount) );
		
			// ----------------- START THREAD 2 ----------------- 
			Future<Object[]> future5e422485_2 = pool.submit( new Worker5e422485_2(values3) );
		
			// ----------------- AWAIT THREAD 0 ----------------- 
			future5e422485_0.get();
			// ----------------- AWAIT THREAD 1 ----------------- 
			future5e422485_1.get();
			// ----------------- AWAIT THREAD 2 ----------------- 
			future5e422485_2.get();
			pool.shutdown();
		}
	...
	private static void quickSort(??? values, ??? start, ??? stop) {
		// =========== START PARALLEL WORKER DEFINITIONS ============ 
		class Worker189f25ec_0 implements Callable<Object[]> {
			private ??? values;
			private ??? start;
			private ??? p;
			public Worker189f25ec_0(??? values, ??? start, ??? p) {
				this.values = values;
				this.start = start;
				this.p = p;
			}
		...
		class Worker189f25ec_1 implements Callable<Object[]> {
			private ??? values;
			private ??? p;
			private ??? stop;
			public Worker189f25ec_1(??? values, ??? p, ??? stop) {
				this.values = values;
				this.p = p;
				this.stop = stop;
			}
		...
		if (stop >= start + 2) {
			...
			try {
				...
				// ----------------- START THREAD 0 ----------------- 
				Future<Object[]> future189f25ec_0 = pool.submit( new Worker189f25ec_0(values, start, p) );
			
				// ----------------- START THREAD 1 ----------------- 
				Future<Object[]> future189f25ec_1 = pool.submit( new Worker189f25ec_1(values, p, stop) );
			
				// ----------------- AWAIT THREAD 0 ----------------- 
				future189f25ec_0.get();
				// ----------------- AWAIT THREAD 1 ----------------- 
				future189f25ec_1.get();
				pool.shutdown();
			}
			catch (Exception ex) { System.err.println(ex.getMessage()); ex.printStackTrace(); }
			// ========================================================== 
