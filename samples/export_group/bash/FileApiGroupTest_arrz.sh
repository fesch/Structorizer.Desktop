...
# Copyright (C) 2026-04-27 Kay Gürtzig 
# License: GPLv3-link 
...
function generateRandomNumberFile() {
    ...
    if (( ${fileNo} <= 0 ))
    then
        result10bf9331=${fileNo}
        return 0
    fi

    trap4007f65b=$( trap -p EXIT )
    if [ -z "$trap4007f65b" ] ; then trap4007f65b="-"; else trap4007f65b=${trap4007f65b:8}; trap4007f65b=${trap4007f65b% *}; fi
    if [ "${trap4007f65b:0:1}" = "'" ] ; then trap4007f65b=${trap4007f65b:1} ; trap4007f65b=${trap4007f65b%\'*}; fi
    trap "finally4007f65b trapped" EXIT
    { # try
    ...
    trap "${trap4007f65b}" EXIT
    { # finally
        finally4007f65b okay
    }
    result10bf9331=${count}
}
