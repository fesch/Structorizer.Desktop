...
public class SORTING_TEST_MAIN {
	...
	public static void main(String[] args) {
		...
		class Worker82e1bc46_0 implements Callable<Object[]> {
			private int[] values1;
			public Worker82e1bc46_0(int[] values1) {
		...
		class Worker82e1bc46_1 implements Callable<Object[]> {
			private int[] values2;
			private ??? elementCount;
			public Worker82e1bc46_1(int[] values2, ??? elementCount) {
		...
		class Worker82e1bc46_2 implements Callable<Object[]> {
			private int[] values3;
			public Worker82e1bc46_2(int[] values3) {
		...
		try {
			...
			// ----------------- START THREAD 0 ----------------- 
			Future<Object[]> future82e1bc46_0 = pool.submit( new Worker82e1bc46_0(values1) );
		
			// ----------------- START THREAD 1 ----------------- 
			Future<Object[]> future82e1bc46_1 = pool.submit( new Worker82e1bc46_1(values2, elementCount) );
		
			// ----------------- START THREAD 2 ----------------- 
			Future<Object[]> future82e1bc46_2 = pool.submit( new Worker82e1bc46_2(values3) );
		
			// ----------------- AWAIT THREAD 0 ----------------- 
			future82e1bc46_0.get();
			// ----------------- AWAIT THREAD 1 ----------------- 
			future82e1bc46_1.get();
			// ----------------- AWAIT THREAD 2 ----------------- 
			future82e1bc46_2.get();
			pool.shutdown();
		}
	...
	private static void quickSort(??? values, ??? start, ??? stop) {
		// =========== START PARALLEL WORKER DEFINITIONS ============ 
		class Worker976e9184_0 implements Callable<Object[]> {
			private ??? values;
			private ??? start;
			private ??? p;
			public Worker976e9184_0(??? values, ??? start, ??? p) {
				this.values = values;
				this.start = start;
				this.p = p;
			}
		...
		class Worker976e9184_1 implements Callable<Object[]> {
			private ??? values;
			private ??? p;
			private ??? stop;
			public Worker976e9184_1(??? values, ??? p, ??? stop) {
				this.values = values;
				this.p = p;
				this.stop = stop;
			}
		...
		if (stop >= start + 2) {
			...
			try {
				...
				// ----------------- START THREAD 0 ----------------- 
				Future<Object[]> future976e9184_0 = pool.submit( new Worker976e9184_0(values, start, p) );
			
				// ----------------- START THREAD 1 ----------------- 
				Future<Object[]> future976e9184_1 = pool.submit( new Worker976e9184_1(values, p, stop) );
			
				// ----------------- AWAIT THREAD 0 ----------------- 
				future976e9184_0.get();
				// ----------------- AWAIT THREAD 1 ----------------- 
				future976e9184_1.get();
				pool.shutdown();
			}
			catch (Exception ex) { System.err.println(ex.getMessage()); ex.printStackTrace(); }
			// ========================================================== 
