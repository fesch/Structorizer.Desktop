...
public class SORTING_TEST_MAIN {
	...
	public static void main(String[] args) {
		...
		class Worker42f4bc25_0 implements Callable<Object[]> {
			private int[] values1;
			public Worker42f4bc25_0(int[] values1) {
		...
		class Worker42f4bc25_1 implements Callable<Object[]> {
			private int[] values2;
			private ??? elementCount;
			public Worker42f4bc25_1(int[] values2, ??? elementCount) {
		...
		class Worker42f4bc25_2 implements Callable<Object[]> {
			private int[] values3;
			public Worker42f4bc25_2(int[] values3) {
		...
		try {
			...
			// ----------------- START THREAD 0 ----------------- 
			Future<Object[]> future42f4bc25_0 = pool.submit( new Worker42f4bc25_0(values1) );
		
			// ----------------- START THREAD 1 ----------------- 
			Future<Object[]> future42f4bc25_1 = pool.submit( new Worker42f4bc25_1(values2, elementCount) );
		
			// ----------------- START THREAD 2 ----------------- 
			Future<Object[]> future42f4bc25_2 = pool.submit( new Worker42f4bc25_2(values3) );
		
			// ----------------- AWAIT THREAD 0 ----------------- 
			future42f4bc25_0.get();
			// ----------------- AWAIT THREAD 1 ----------------- 
			future42f4bc25_1.get();
			// ----------------- AWAIT THREAD 2 ----------------- 
			future42f4bc25_2.get();
			pool.shutdown();
		}
	...
	private static void quickSort(??? values, ??? start, ??? stop) {
		// =========== START PARALLEL WORKER DEFINITIONS ============ 
		class Worker460cccb6_0 implements Callable<Object[]> {
			private ??? values;
			private ??? start;
			private ??? p;
			public Worker460cccb6_0(??? values, ??? start, ??? p) {
				this.values = values;
				this.start = start;
				this.p = p;
			}
		...
		class Worker460cccb6_1 implements Callable<Object[]> {
			private ??? values;
			private ??? p;
			private ??? stop;
			public Worker460cccb6_1(??? values, ??? p, ??? stop) {
				this.values = values;
				this.p = p;
				this.stop = stop;
			}
		...
		if (stop >= start + 2) {
			...
			try {
				...
				// ----------------- START THREAD 0 ----------------- 
				Future<Object[]> future460cccb6_0 = pool.submit( new Worker460cccb6_0(values, start, p) );
			
				// ----------------- START THREAD 1 ----------------- 
				Future<Object[]> future460cccb6_1 = pool.submit( new Worker460cccb6_1(values, p, stop) );
			
				// ----------------- AWAIT THREAD 0 ----------------- 
				future460cccb6_0.get();
				// ----------------- AWAIT THREAD 1 ----------------- 
				future460cccb6_1.get();
				pool.shutdown();
			}
			catch (Exception ex) { System.err.println(ex.getMessage()); ex.printStackTrace(); }
			// ========================================================== 
