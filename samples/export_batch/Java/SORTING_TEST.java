...
public class SORTING_TEST_MAIN {
	...
	public static void main(String[] args) {
		...
		class Worker9686dab4_0 implements Callable<Object[]> {
			private int[] values1;
			public Worker9686dab4_0(int[] values1) {
		...
		class Worker9686dab4_1 implements Callable<Object[]> {
			private ???[] values2;
			private ??? elementCount;
			public Worker9686dab4_1(???[] values2, ??? elementCount) {
		...
		class Worker9686dab4_2 implements Callable<Object[]> {
			private ???[] values3;
			public Worker9686dab4_2(???[] values3) {
		...
		try {
			...
			// ----------------- START THREAD 0 ----------------- 
			Future<Object[]> future9686dab4_0 = pool.submit( new Worker9686dab4_0(values1) );
		
			// ----------------- START THREAD 1 ----------------- 
			Future<Object[]> future9686dab4_1 = pool.submit( new Worker9686dab4_1(values2, elementCount) );
		
			// ----------------- START THREAD 2 ----------------- 
			Future<Object[]> future9686dab4_2 = pool.submit( new Worker9686dab4_2(values3) );
		
			// ----------------- AWAIT THREAD 0 ----------------- 
			future9686dab4_0.get();
			// ----------------- AWAIT THREAD 1 ----------------- 
			future9686dab4_1.get();
			// ----------------- AWAIT THREAD 2 ----------------- 
			future9686dab4_2.get();
			pool.shutdown();
		}
	...
	private static void quickSort(??? values, ??? start, ??? stop) {
		// =========== START PARALLEL WORKER DEFINITIONS ============ 
		class Workere4b98437_0 implements Callable<Object[]> {
			private ??? values;
			private ??? start;
			private ??? p;
			public Workere4b98437_0(??? values, ??? start, ??? p) {
				this.values = values;
				this.start = start;
				this.p = p;
			}
		...
		class Workere4b98437_1 implements Callable<Object[]> {
			private ??? values;
			private ??? p;
			private ??? stop;
			public Workere4b98437_1(??? values, ??? p, ??? stop) {
				this.values = values;
				this.p = p;
				this.stop = stop;
			}
		...
		if (stop >= start + 2) {
			...
			try {
				...
				// ----------------- START THREAD 0 ----------------- 
				Future<Object[]> futuree4b98437_0 = pool.submit( new Workere4b98437_0(values, start, p) );
			
				// ----------------- START THREAD 1 ----------------- 
				Future<Object[]> futuree4b98437_1 = pool.submit( new Workere4b98437_1(values, p, stop) );
			
				// ----------------- AWAIT THREAD 0 ----------------- 
				futuree4b98437_0.get();
				// ----------------- AWAIT THREAD 1 ----------------- 
				futuree4b98437_1.get();
				pool.shutdown();
			}
			catch (Exception ex) { System.err.println(ex.getMessage()); ex.printStackTrace(); }
			// ========================================================== 
