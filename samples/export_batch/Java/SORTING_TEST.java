...
public class SORTING_TEST_MAIN {
	...
	public static void main(String[] args) {
		...
		class Worker1cab0bfb_0 implements Callable<Object[]> {
			private int[] values1;
			public Worker1cab0bfb_0(int[] values1) {
		...
		class Worker1cab0bfb_1 implements Callable<Object[]> {
			private int[] values2;
			private ??? elementCount;
			public Worker1cab0bfb_1(int[] values2, ??? elementCount) {
		...
		class Worker1cab0bfb_2 implements Callable<Object[]> {
			private int[] values3;
			public Worker1cab0bfb_2(int[] values3) {
		...
		try {
			...
			// ----------------- START THREAD 0 ----------------- 
			Future<Object[]> future1cab0bfb_0 = pool.submit( new Worker1cab0bfb_0(values1) );
		
			// ----------------- START THREAD 1 ----------------- 
			Future<Object[]> future1cab0bfb_1 = pool.submit( new Worker1cab0bfb_1(values2, elementCount) );
		
			// ----------------- START THREAD 2 ----------------- 
			Future<Object[]> future1cab0bfb_2 = pool.submit( new Worker1cab0bfb_2(values3) );
		
			// ----------------- AWAIT THREAD 0 ----------------- 
			future1cab0bfb_0.get();
			// ----------------- AWAIT THREAD 1 ----------------- 
			future1cab0bfb_1.get();
			// ----------------- AWAIT THREAD 2 ----------------- 
			future1cab0bfb_2.get();
			pool.shutdown();
		}
	...
	private static void quickSort(??? values, ??? start, ??? stop) {
		// =========== START PARALLEL WORKER DEFINITIONS ============ 
		class Worker32464a14_0 implements Callable<Object[]> {
			private ??? values;
			private ??? start;
			private ??? p;
			public Worker32464a14_0(??? values, ??? start, ??? p) {
				this.values = values;
				this.start = start;
				this.p = p;
			}
		...
		class Worker32464a14_1 implements Callable<Object[]> {
			private ??? values;
			private ??? p;
			private ??? stop;
			public Worker32464a14_1(??? values, ??? p, ??? stop) {
				this.values = values;
				this.p = p;
				this.stop = stop;
			}
		...
		if (stop >= start + 2) {
			...
			??? p = (randGen.nextInt() % (stop-start)) + start;
			...
			try {
				...
				// ----------------- START THREAD 0 ----------------- 
				Future<Object[]> future32464a14_0 = pool.submit( new Worker32464a14_0(values, start, p) );
			
				// ----------------- START THREAD 1 ----------------- 
				Future<Object[]> future32464a14_1 = pool.submit( new Worker32464a14_1(values, p, stop) );
			
				// ----------------- AWAIT THREAD 0 ----------------- 
				future32464a14_0.get();
				// ----------------- AWAIT THREAD 1 ----------------- 
				future32464a14_1.get();
				pool.shutdown();
			}
			catch (Exception ex) { System.err.println(ex.getMessage()); ex.printStackTrace(); }
			// ========================================================== 
