...
Public Function readNumbers(fileName AS String, numbers AS array of integer, maxNumbers AS integer) As integer
  ...
  Try
    Do While NOT fileEOF(fileNo) AND nNumbers < maxNumbers
      ...
      nNumbers = nNumbers + 1
    Loop
  Catch ex1d2e4506 As Exception
    Dim error As String = ex1d2e4506.ToString()
    Throw New Exception()
  Finally
    fileClose(fileNo)
  End Try
...
Rem   fileClose(fileNo) 
  ...
  Catch ex61ffd071 As Exception
    Dim failure As String = ex61ffd071.ToString()
    PRINT failure
    Stop
  End Try
  sum = 0.0
...
If fileNo > 0 Then
  ...
  Catch ex69534998 As Exception
    Dim failure As String = ex69534998.ToString()
    PRINT failure
  End Try
  If nObtained > 0 Then
    min = numberArray(0)
