...
Public Function readNumbers(fileName As String, numbers As integer(50), maxNumbers As integer) As integer
  ...
  Try
    Do While NOT fileEOF(fileNo) AND nNumbers < maxNumbers
      ...
      nNumbers = nNumbers + 1
    Loop
  Catch ex4abdb505 As Exception
    Dim error As String = ex4abdb505.ToString()
    Throw 
  Finally
    fileClose(fileNo)
  End Try
...
Rem   fileClose(fileNo) 
  ...
  Catch ex7113b13f As Exception
    Dim failure As String = ex7113b13f.ToString()
    PRINT failure
...
If fileNo > 0 Then
  ...
  Catch ex13c27452 As Exception
    Dim failure As String = ex13c27452.ToString()
    PRINT failure
  End Try
  If nObtained > 0 Then
    min = numberArray(0)
