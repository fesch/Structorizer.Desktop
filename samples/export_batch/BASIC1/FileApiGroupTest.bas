...
Public Function readNumbers(fileName As String, numbers As integer(50), maxNumbers As integer) As integer
  ...
  Try
    Do While NOT fileEOF(fileNo) AND nNumbers < maxNumbers
      ...
      nNumbers = nNumbers + 1
    Loop
  Catch ex92ab77ab As Exception
    Dim error As String = ex92ab77ab.ToString()
    Throw New Exception()
  Finally
    fileClose(fileNo)
  End Try
...
Rem   fileClose(fileNo) 
  ...
  Catch ex158da03e As Exception
    Dim failure As String = ex158da03e.ToString()
    PRINT failure
    Stop
  End Try
  sum = 0.0
...
If fileNo > 0 Then
  ...
  Catch ex227d9b15 As Exception
    Dim failure As String = ex227d9b15.ToString()
    PRINT failure
  End Try
  If nObtained > 0 Then
    min = numberArray(0)
