...
public class SORTING_TEST_MAIN {
	...
	public static void main(String[] args) {
		...
		class Worker7e84aaaa_0 implements Callable<Object[]> {
			private int[] values1;
			public Worker7e84aaaa_0(int[] values1) {
		...
		class Worker7e84aaaa_1 implements Callable<Object[]> {
			private int[] values2;
			private ??? elementCount;
			public Worker7e84aaaa_1(int[] values2, ??? elementCount) {
		...
		class Worker7e84aaaa_2 implements Callable<Object[]> {
			private int[] values3;
			public Worker7e84aaaa_2(int[] values3) {
		...
		try {
			...
			// ----------------- START THREAD 0 ----------------- 
			Future<Object[]> future7e84aaaa_0 = pool.submit( new Worker7e84aaaa_0(values1) );
		
			// ----------------- START THREAD 1 ----------------- 
			Future<Object[]> future7e84aaaa_1 = pool.submit( new Worker7e84aaaa_1(values2, elementCount) );
		
			// ----------------- START THREAD 2 ----------------- 
			Future<Object[]> future7e84aaaa_2 = pool.submit( new Worker7e84aaaa_2(values3) );
		
			// ----------------- AWAIT THREAD 0 ----------------- 
			future7e84aaaa_0.get();
			// ----------------- AWAIT THREAD 1 ----------------- 
			future7e84aaaa_1.get();
			// ----------------- AWAIT THREAD 2 ----------------- 
			future7e84aaaa_2.get();
			pool.shutdown();
		}
	...
	private static void quickSort(??? values, ??? start, ??? stop) {
		// =========== START PARALLEL WORKER DEFINITIONS ============ 
		class Workerb962b98_0 implements Callable<Object[]> {
			private ??? values;
			private ??? start;
			private ??? p;
			public Workerb962b98_0(??? values, ??? start, ??? p) {
				this.values = values;
				this.start = start;
				this.p = p;
			}
		...
		class Workerb962b98_1 implements Callable<Object[]> {
			private ??? values;
			private ??? p;
			private ??? stop;
			public Workerb962b98_1(??? values, ??? p, ??? stop) {
				this.values = values;
				this.p = p;
				this.stop = stop;
			}
		...
		if (stop >= start + 2) {
			...
			try {
				...
				// ----------------- START THREAD 0 ----------------- 
				Future<Object[]> futureb962b98_0 = pool.submit( new Workerb962b98_0(values, start, p) );
			
				// ----------------- START THREAD 1 ----------------- 
				Future<Object[]> futureb962b98_1 = pool.submit( new Workerb962b98_1(values, p, stop) );
