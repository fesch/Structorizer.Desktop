...
public class SORTING_TEST_MAIN {
	...
	public static void main(String[] args) {
		...
		class Worker882e3cf9_0 implements Callable<Object[]> {
			private int[] values1;
			public Worker882e3cf9_0(int[] values1) {
		...
		class Worker882e3cf9_1 implements Callable<Object[]> {
			private ???[] values2;
			private ??? elementCount;
			public Worker882e3cf9_1(???[] values2, ??? elementCount) {
		...
		class Worker882e3cf9_2 implements Callable<Object[]> {
			private ???[] values3;
			public Worker882e3cf9_2(???[] values3) {
		...
		try {
			...
			// ----------------- START THREAD 0 ----------------- 
			Future<Object[]> future882e3cf9_0 = pool.submit( new Worker882e3cf9_0(values1) );
		
			// ----------------- START THREAD 1 ----------------- 
			Future<Object[]> future882e3cf9_1 = pool.submit( new Worker882e3cf9_1(values2, elementCount) );
		
			// ----------------- START THREAD 2 ----------------- 
			Future<Object[]> future882e3cf9_2 = pool.submit( new Worker882e3cf9_2(values3) );
		
			Object[] results;
			// ----------------- AWAIT THREAD 0 ----------------- 
			results = future882e3cf9_0.get();
			// ----------------- AWAIT THREAD 1 ----------------- 
			results = future882e3cf9_1.get();
			// ----------------- AWAIT THREAD 2 ----------------- 
			results = future882e3cf9_2.get();
			pool.shutdown();
		}
	...
	private static int quickSort(??? values, ??? start, ??? stop) {
		// =========== START PARALLEL WORKER DEFINITIONS ============ 
		class Worker87dcc0ed_0 implements Callable<Object[]> {
			private ??? values;
			private ??? start;
			private ??? p;
			public Worker87dcc0ed_0(??? values, ??? start, ??? p) {
				this.values = values;
				this.start = start;
				this.p = p;
			}
		...
		class Worker87dcc0ed_1 implements Callable<Object[]> {
			private ??? values;
			private ??? p;
			private ??? stop;
			public Worker87dcc0ed_1(??? values, ??? p, ??? stop) {
				this.values = values;
				this.p = p;
				this.stop = stop;
			}
		...
		if (stop >= start + 2) {
			...
			try {
				...
				// ----------------- START THREAD 0 ----------------- 
				Future<Object[]> future87dcc0ed_0 = pool.submit( new Worker87dcc0ed_0(values, start, p) );
			
				// ----------------- START THREAD 1 ----------------- 
				Future<Object[]> future87dcc0ed_1 = pool.submit( new Worker87dcc0ed_1(values, p, stop) );
			
				Object[] results;
				// ----------------- AWAIT THREAD 0 ----------------- 
				results = future87dcc0ed_0.get();
				// ----------------- AWAIT THREAD 1 ----------------- 
				results = future87dcc0ed_1.get();
				pool.shutdown();
			}
			catch (Exception ex) { System.err.println(ex.getMessage()); ex.printStackTrace(); }
			// ========================================================== 
