...
public class SORTING_TEST_MAIN {
	...
	public static void main(String[] args) {
		...
		class Workerce3f34d1_0 implements Callable<Object[]> {
			private int[] values1;
			public Workerce3f34d1_0(int[] values1) {
		...
		class Workerce3f34d1_1 implements Callable<Object[]> {
			private ???[] values2;
			private ??? elementCount;
			public Workerce3f34d1_1(???[] values2, ??? elementCount) {
		...
		class Workerce3f34d1_2 implements Callable<Object[]> {
			private ???[] values3;
			public Workerce3f34d1_2(???[] values3) {
		...
		try {
			...
			// ----------------- START THREAD 0 ----------------- 
			Future<Object[]> futurece3f34d1_0 = pool.submit( new Workerce3f34d1_0(values1) );
		
			// ----------------- START THREAD 1 ----------------- 
			Future<Object[]> futurece3f34d1_1 = pool.submit( new Workerce3f34d1_1(values2, elementCount) );
		
			// ----------------- START THREAD 2 ----------------- 
			Future<Object[]> futurece3f34d1_2 = pool.submit( new Workerce3f34d1_2(values3) );
		
			// ----------------- AWAIT THREAD 0 ----------------- 
			futurece3f34d1_0.get();
			// ----------------- AWAIT THREAD 1 ----------------- 
			futurece3f34d1_1.get();
			// ----------------- AWAIT THREAD 2 ----------------- 
			futurece3f34d1_2.get();
			pool.shutdown();
		}
	...
	private static void quickSort(??? values, ??? start, ??? stop) {
		// =========== START PARALLEL WORKER DEFINITIONS ============ 
		class Worker68b9aebd_0 implements Callable<Object[]> {
			private ??? values;
			private ??? start;
			private ??? p;
			public Worker68b9aebd_0(??? values, ??? start, ??? p) {
				this.values = values;
				this.start = start;
				this.p = p;
			}
		...
		class Worker68b9aebd_1 implements Callable<Object[]> {
			private ??? values;
			private ??? p;
			private ??? stop;
			public Worker68b9aebd_1(??? values, ??? p, ??? stop) {
				this.values = values;
				this.p = p;
				this.stop = stop;
			}
		...
		if (stop >= start + 2) {
			...
			try {
				...
				// ----------------- START THREAD 0 ----------------- 
				Future<Object[]> future68b9aebd_0 = pool.submit( new Worker68b9aebd_0(values, start, p) );
			
				// ----------------- START THREAD 1 ----------------- 
				Future<Object[]> future68b9aebd_1 = pool.submit( new Worker68b9aebd_1(values, p, stop) );
			
				// ----------------- AWAIT THREAD 0 ----------------- 
				future68b9aebd_0.get();
				// ----------------- AWAIT THREAD 1 ----------------- 
				future68b9aebd_1.get();
				pool.shutdown();
			}
			catch (Exception ex) { System.err.println(ex.getMessage()); ex.printStackTrace(); }
			// ========================================================== 
