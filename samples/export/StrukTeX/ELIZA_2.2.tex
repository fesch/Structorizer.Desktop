...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(185,68)[checkGoodBye]
  \assign{%
    \begin{declaration}[Parameters:]
...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(157,87)[checkRepetition]
  \assign{%
    \begin{declaration}[Parameters:]
  ...
  \ifthenelse{10}{2}{\(length(newInput)\ >\ 4\)}{ja}{nein}
    ...
    \while{\pKey{for}\(i\ \gets\ 1\)\pKey{to}\(length(history)-1\)}
      ...
      \ifthenelse{3}{1}{\(newInput\ =\ history[i]\)}{ja}{nein}
        \assign{\(hasOccurred\ \gets\ true\)}
      \change
      \ifend
    \whileend
    \assign{\(history[history[0]+1]\ \gets\ newInput\)}
    \assign{\(history[0]\ \gets\ (history[0]\ +\ 1)\ \)\pKey{mod}\(\ (length(history)\ -\ 1)\)}
  \change
  \ifend
...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(243,69)[normalizeInput]
  \assign{%
    \begin{declaration}[Parameters:]
      \description{\pVar{sentence}}{type: \(string\)}
    \end{declaration}
...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(126,150)[setupKeywords]
  \assign{%
    \begin{declaration}[Result type:]
...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(303,342)[ELIZA]
  \assign{%
    \begin{declaration}[Requires:]
      \description{KeyMapEntry}{}
    \end{declaration}
