...
function partition(values: {type?}; start: {type?}; stop: {type?}; p: {type?}): Longint;
...
function testSorted(numbers: {type?}): Boolean;
...
{ the root. }
function buildMaxHeap(heap: {type?}): Integer;
...
function quickSort(values: {type?}; start: {type?}; stop: {type?}): Integer;
...
{ algorithm }
function heapSort(values: {type?}): Integer;
