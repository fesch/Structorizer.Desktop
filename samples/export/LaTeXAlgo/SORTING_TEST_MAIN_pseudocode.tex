...
\label{maxHeapify}
\COMMENT{ Given a max-heap '{}heap´ with element at index '{}i´ possibly }\\
...
\label{partition}
\COMMENT{ Partitions array '{}values´ between indices '{}start´ und '{}stop´-1 with }\\
\COMMENT{ respect to the pivot element initially at index '{}p´ into smaller }\\
...
\PROCEDURE{partition}{values, start, stop, p}
  ...
  \COMMENT{ 3. i \textless start → values[i] ≤ pivot }\\
  \COMMENT{ 4. stop \textless i → pivot \textless values[i] }\\
...
\label{quickSort}
\COMMENT{ Recursively sorts a subrange of the given array '{}values´.  }\\
...
\label{heapSort}
\COMMENT{ Sorts the array '{}values´ of numbers according to he heap sort }\\
\COMMENT{ algorithm }\\
...
\begin{pseudocode}{SORTING_TEST_MAIN}{ }
\label{SORTING_TEST_MAIN}
