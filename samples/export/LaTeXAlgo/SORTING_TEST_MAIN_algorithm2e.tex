...
\DontPrintSemicolon
\title{Structorizer LaTeX pseudocode Export of SORTING_TEST.arrz}
\author{Kay Gürtzig}
\date{08.06.2021}
...
\tcc{ Given a max-heap '{}heap´ with element at index '{}i´ possibly }
...
\tcc{ Partitions array '{}values´ between indices '{}start´ und '{}stop´-1 with }
\tcc{ respect to the pivot element initially at index '{}p´ into smaller }
...
\KwResult{int}
  ...
  \tcc{ 1. p = start - 1 }
  \tcc{ 2. pivot = values[p] }
  \tcc{ 3. i \textless start → values[i] ≤ pivot }
  \tcc{ 4. stop \textless i → pivot \textless values[i] }
...
\tcc{ Recursively sorts a subrange of the given array '{}values´.  }
...
\KwData{\(stop\): ?}
  ...
  \If{\(stop\geq\ start+2\)}{
    ...
    \(p\leftarrow{}partition(values,start,stop,p)\)\;
...
\tcc{ Sorts the array '{}values´ of numbers according to he heap sort }
\tcc{ algorithm }
...
\Prog{\FuncSty{SORTING_TEST_MAIN}}{
  ...
  \(ok1\leftarrow{}testSorted(values1)\)\;
  \(ok2\leftarrow{}testSorted(values2)\)\;
  \(ok3\leftarrow{}testSorted(values3)\)\;
