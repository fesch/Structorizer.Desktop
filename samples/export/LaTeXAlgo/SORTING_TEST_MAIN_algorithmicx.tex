...
\State \Comment{ Given a max-heap '{}heap´ with element at index '{}i´ possibly }
...
\State \Comment{ Partitions array '{}values´ between indices '{}start´ und '{}stop´-1 with }
\State \Comment{ respect to the pivot element initially at index '{}p´ into smaller }
...
\State \Comment{ might still be avoided) but it is pretty clear. }
  ...
  \State \Comment{ 3. i \textless start → values[i] ≤ pivot }
  \State \Comment{ 4. stop \textless i → pivot \textless values[i] }
...
\State \Comment{ Recursively sorts a subrange of the given array '{}values´.  }
...
\State \Comment{ Sorts the array '{}values´ of numbers according to he heap sort }
