...
50 REM Generated by Structorizer 3.30-06 
60 REM  
...
2040 FUNCTION buildMaxHeap(heap)
...
2110 END FUNCTION
2120 REM  
2130 REM Recursively sorts a subrange of the given array 'values´.  
2140 REM start is the first index of the subsequence to be sorted, 
2150 REM stop is the index BEHIND the subsequence to be sorted. 
2160 REM TODO: Add type-specific suffixes where necessary! 
2170 FUNCTION quickSort(values, start, stop)
2180   REM TODO: add the respective type suffixes to your variable names if required 
2190   REM At least 2 elements? (Less don't make sense.) 
2200   IF stop >= start + 2 THEN
2210     REM Select a pivot element, be p its index. 
2220     REM (here: randomly chosen element out of start ... stop-1) 
2230     LET p = random(stop-start) + start
2240     REM Partition the array into smaller and greater elements 
2250     REM Get the resulting (and final) position of the pivot element 
2260     LET p = partition(values, start, stop, p)
2270     REM Sort subsequances separately and independently ... 
2280 
2290     REM ========================================================== 
2300     REM ================= START PARALLEL SECTION ================= 
2310     REM ========================================================== 
2320     REM TODO: add the necessary code to run the threads concurrently 
2330 
2340       REM ----------------- START THREAD 0 ----------------- 
2350         REM Sort left (lower) array part 
2360         CALL quickSort(values, start, p)
2370       REM ------------------ END THREAD 0 ------------------ 
2380 
2390 
2400       REM ----------------- START THREAD 1 ----------------- 
2410         REM Sort right (higher) array part 
2420         CALL quickSort(values, p+1, stop)
2430       REM ------------------ END THREAD 1 ------------------ 
2440 
2450     REM ========================================================== 
2460     REM ================== END PARALLEL SECTION ================== 
2470     REM ========================================================== 
2480 
2490   END IF
2500   RETURN 0
2510 END FUNCTION
2520 REM  
2530 REM Sorts the array 'values´ of numbers according to he heap sort 
2540 REM algorithm 
2550 REM TODO: Add type-specific suffixes where necessary! 
2560 FUNCTION heapSort(values)
2570   REM TODO: add the respective type suffixes to your variable names if required 
2580   CALL buildMaxHeap(values)
2590   LET heapRange = length(values)
2600   FOR k = heapRange - 1 TO 1 STEP -1
2610     LET heapRange = heapRange - 1
2620     REM Swap the maximum value (root of the heap) to the heap end 
2630     LET maximum = values(0)
2640     LET values(0) = values(heapRange)
2650     LET values(heapRange) = maximum
2660     CALL maxHeapify(values, 0, heapRange)
2670   NEXT k
2680   RETURN 0
2690 END FUNCTION
