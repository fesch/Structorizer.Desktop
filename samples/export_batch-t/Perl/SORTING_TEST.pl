...
sub quickSort {
    ...
    if stop >= start + 2 {
        ...
        # Requires at least Perl 5.8 and version threads 2.07 
        {
            ...
            # ----------------- START THREAD 0 ----------------- 
            my $thr4d41cee_0 = threads->create(sub {
                my $values = $_[0];
                my $start = $_[1];
                my $p = $_[2];
            ...
            # ----------------- START THREAD 1 ----------------- 
            my $thr4d41cee_1 = threads->create(sub {
                my $values = $_[0];
                my $p = $_[1];
                my $stop = $_[2];
...
# Requires at least Perl 5.8 and version threads 2.07 
{
    ...
    # ----------------- START THREAD 0 ----------------- 
    my $thr48503868_0 = threads->create(sub {
        my $values1 = $_[0];
        bubbleSort(values1);
    }, (values1));
    ...
    # ----------------- START THREAD 1 ----------------- 
    my $thr48503868_1 = threads->create(sub {
        my $values2 = $_[0];
        my $elementCount = $_[1];
    ...
    # ----------------- START THREAD 2 ----------------- 
    my $thr48503868_2 = threads->create(sub {
        my $values3 = $_[0];
        heapSort(values3);
    }, (values3));
