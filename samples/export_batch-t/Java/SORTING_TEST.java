...
public class SORTING_TEST_MAIN
{
	...
	public static void main(String[] args)
	{
		...
		class Worker27d415d9_0 implements Callable<Object[]> {
			private int[] values1;
			public Worker27d415d9_0(int[] values1) {
		...
		class Worker27d415d9_1 implements Callable<Object[]> {
			private int[] values2;
			private ??? elementCount;
			public Worker27d415d9_1(int[] values2, ??? elementCount) {
		...
		class Worker27d415d9_2 implements Callable<Object[]> {
			private int[] values3;
			public Worker27d415d9_2(int[] values3) {
		...
		try {
			...
			// ----------------- START THREAD 0 ----------------- 
			Future<Object[]> future27d415d9_0 = pool.submit( new Worker27d415d9_0(values1) );
		
			// ----------------- START THREAD 1 ----------------- 
			Future<Object[]> future27d415d9_1 = pool.submit( new Worker27d415d9_1(values2, elementCount) );
		
			// ----------------- START THREAD 2 ----------------- 
			Future<Object[]> future27d415d9_2 = pool.submit( new Worker27d415d9_2(values3) );
		
			// ----------------- AWAIT THREAD 0 ----------------- 
			future27d415d9_0.get();
			// ----------------- AWAIT THREAD 1 ----------------- 
			future27d415d9_1.get();
			// ----------------- AWAIT THREAD 2 ----------------- 
			future27d415d9_2.get();
			pool.shutdown();
		}
	...
	{
		// =========== START PARALLEL WORKER DEFINITIONS ============ 
		class Worker44a3ec6b_0 implements Callable<Object[]> {
			private ??? values;
			private ??? start;
			private ??? p;
			public Worker44a3ec6b_0(??? values, ??? start, ??? p) {
				this.values = values;
				this.start = start;
				this.p = p;
			}
		...
		class Worker44a3ec6b_1 implements Callable<Object[]> {
			private ??? values;
			private ??? p;
			private ??? stop;
			public Worker44a3ec6b_1(??? values, ??? p, ??? stop) {
				this.values = values;
				this.p = p;
				this.stop = stop;
			}
		...
		if (stop >= start + 2)
		{
			...
			try {
				...
				// ----------------- START THREAD 0 ----------------- 
				Future<Object[]> future44a3ec6b_0 = pool.submit( new Worker44a3ec6b_0(values, start, p) );
			
				// ----------------- START THREAD 1 ----------------- 
				Future<Object[]> future44a3ec6b_1 = pool.submit( new Worker44a3ec6b_1(values, p, stop) );
			
				// ----------------- AWAIT THREAD 0 ----------------- 
				future44a3ec6b_0.get();
				// ----------------- AWAIT THREAD 1 ----------------- 
				future44a3ec6b_1.get();
				pool.shutdown();
			}
			catch (Exception ex) { System.err.println(ex.getMessage()); ex.printStackTrace(); }
			// ========================================================== 
