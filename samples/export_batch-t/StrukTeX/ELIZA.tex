...
\begin{struktogramm}(-14,0)[checkRepetition]
  ...
  \ifthenelse{6}{6}{\(length(newInput)\ >\ 4\)}{T}{F}
    \assign{\(histDepth\ <-\ length(history.histArray)\)}
    \while{\pKey{for}\(i\ \gets\ 0\)\pKey{to}\(histDepth-1\)}
      ...
      \ifend
    \whileend
    \assign{\(history.histArray[history.histIndex]\ <-\ newInput\)}
    \assign{\(history.histIndex\ <-\ (history.histIndex\ +\ 1)\ mod\ (histDepth)\)}
  \change
  \ifend
...
\begin{struktogramm}(-14,0)[conjugateStrings]
  ...
  \while{\(\forall pair\in flexions\)}
    \assign{\(left\ <-\ ""\)}
    \assign{\(right\ <-\ result\)}
    \assign{\(pos0\ <-\ pos(pair[0],\ right)\)}
    \assign{\(pos1\ <-\ pos(pair[1],\ right)\)}
    \while{\(pos0\ >\ 0\ or\ pos1\ >\ 0\)}
      \assign{\(which\ <-\ 0\)}
      \assign{\(position\ <-\ pos0\)}
      \ifthenelse{6}{6}{\((pos0\ =\ 0)\ or\ ((pos1\ >\ 0)\ and\ (pos1\ <\ pos0))\)}{T}{F}
        \assign{\(which\ <-\ 1\)}
        \assign{\(position\ <-\ pos1\)}
      \change
      \ifend
      \assign{\(left\ <-\ left\ +\ copy(right,\ 1,\ position-1)\ +\ pair[1\ -\ which]\)}
      \assign{\(right\ <-\ copy(right,\ position\ +\ length(pair[which]),\ length(right))\)}
      \assign{\(pos0\ <-\ pos(pair[0],\ right)\)}
      \assign{\(pos1\ <-\ pos(pair[1],\ right)\)}
    \whileend
  ...
  \whileend
  \while{\(\forall str\in \{"\ \ ",\ "|"\}\)}
    \assign{\(position\ <-\ pos(str,\ result)\)}
    \while{\(position\ >\ 0\)}
...
\begin{struktogramm}(-14,0)[setupReflexions]
  ...
  \assign{\(reflexions[3]\ <-\ {"\ yours\ ",\ "\ mine\ "}\)}
  \assign{\(reflexions[4]\ <-\ {"\ yourself\ ",\ "\ myself\ "}\)}
  \assign{\(reflexions[5]\ <-\ {"\ your\ ",\ "\ my\ "}\)}
  \assign{\(reflexions[6]\ <-\ {"\ i\'ve\ ",\ "\ you\'ve\ "}\)}
  \assign{\(reflexions[7]\ <-\ {"\ i\'m\ ",\ "\ you\'re\ "}\)}
  \assign{\(reflexions[8]\ <-\ {"\ me\ ",\ "\ |you\ "}\)}
...
\begin{struktogramm}(-14,0)[ELIZA]
  ...
  \assign{\(OUTPUT\ "*\ Version:\ 2.4\ (2022-01-11)"\)}
  \assign{\(OUTPUT\ "*\ (Requires\ at\ least\ Structorizer\ 3.30-03\ to\ run)"\)}
  \assign{\(OUTPUT\ "**********************************"\)}
  \assign{\(history\ <-\ History{{"",\ "",\ "",\ "",\ ""},\ 0}\)}
