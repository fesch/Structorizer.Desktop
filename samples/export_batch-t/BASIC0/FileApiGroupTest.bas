...
1380 REM   fileNo <- fileOpen(file_name) 
...
1460   nValues <- readNumbers(file_name, values, 1000)
1470   GOTO 1520
...
1650 REM Generated by Structorizer 3.32-31 
1660 
...
1820   fileNo <- fileOpen(file_name)
...
1930   numberArray <- {}
1940   nObtained <- 0
1950   ON ERROR GOTO 1990
1960   nObtained <- readNumbers(file_name, numberArray, 10000)
1970   GOTO 2010
1980   REM Start of error handler, FIXME: variable 'failure' should conatain error info ... 
1990     PRINT failure
2000   REM End of error handler, resume here ... 
2010   ON ERROR GOTO 0
2020   IF nObtained > 0 THEN
2030     min <- numberArray[0]
2040     max <- numberArray[0]
2050     FOR i = 1 TO nObtained-1
2060       IF numberArray[i] < min THEN
2070         min <- numberArray[i]
2080       ELSE
2090         IF numberArray[i] > max THEN
2100           max <- numberArray[i]
2110         END IF
2120       END IF
2130     NEXT i
2140     REM Interval width 
2150     width <- (max - min) * 1.0 / nIntervals
2160     FOR i = 0 TO nObtained - 1
2170       value <- numberArray[i]
2180       k <- 1
2190       DO WHILE k < nIntervals and value > min + k * width
2200         k <- k + 1
2210       LOOP
2220       count[k-1] <- count[k-1] + 1
2230       IF count[k-1] > count[kMaxCount] THEN
2240         kMaxCount <- k-1
2250       END IF
2260     NEXT i
2270     CALL drawBarChart(count, nIntervals)
2280     PRINT "Interval with max count: "; kMaxCount; " ("; count[kMaxCount]; ")"
2290     FOR k = 0 TO nIntervals-1
2300       PRINT count[k]; " numbers in interval "; k; " ("; min + k * width; " ... "; min + (k+1) * width; ")"
2310     NEXT k
2320   ELSE
2330     PRINT "No numbers read."
2340   END IF
2350 END IF
2360 END
2370 REM  
2380 REM Draws a bar chart from the array "values" of size nValues. 
2390 REM Turtleizer must be activated and will scale the chart into a square of 
2400 REM 500 x 500 pixels 
2410 REM Note: The function is not robust against empty array or totally equal values. 
2420 REM TODO: Add type-specific suffixes where necessary! 
2430 SUB drawBarChart(values AS double(50), nValues)
2440   REM TODO: add the respective type suffixes to your variable names if required 
2450   REM  
2460   REM  
2470   REM Used range of the Turtleizer screen 
2480   CONST xSize <- 500
2490   CONST ySize <- 500
2500   kMin <- 0
2510   kMax <- 0
2520   FOR k = 1 TO nValues-1
2530     IF values[k] > values[kMax] THEN
2540       kMax <- k
2550     ELSE
2560       IF values[k] < values[kMin] THEN
2570         kMin <- k
2580       END IF
2590     END IF
2600   NEXT k
2610   valMin <- values[kMin]
2620   valMax <- values[kMax]
2630   yScale <- valMax * 1.0 / (ySize - 1)
2640   yAxis <- ySize - 1
2650   IF valMin < 0 THEN
2660     IF valMax > 0 THEN
2670       yAxis <- valMax * ySize * 1.0 / (valMax - valMin)
2680       yScale <- (valMax - valMin) * 1.0 / (ySize - 1)
2690     ELSE
2700       yAxis <- 1
2710       yScale <- valMin * 1.0 / (ySize - 1)
2720     END IF
2730   END IF
2740   REM draw coordinate axes 
2750   gotoXY(1, ySize - 1)
2760   forward(ySize -1) : REM color = ffffff
2770   penUp()
2780   backward(yAxis) : REM color = ffffff
2790   right(90)
2800   penDown()
2810   forward(xSize -1) : REM color = ffffff
2820   penUp()
2830   backward(xSize-1) : REM color = ffffff
2840   stripeWidth <- xSize / nValues
2850   FOR k = 0 TO nValues-1
2860     stripeHeight <- values[k] * 1.0 / yScale
2870     SELECT CASE k mod 3
2880       CASE 0
2890         setPenColor(255,0,0)
2900       CASE 1
2910         setPenColor(0, 255,0)
2920       CASE 2
2930         setPenColor(0, 0, 255)
2940     END SELECT
2950     fd(1) : REM color = ffffff
2960     left(90)
2970     penDown()
2980     fd(stripeHeight) : REM color = ffffff
2990     right(90)
3000     fd(stripeWidth - 1) : REM color = ffffff
3010     right(90)
3020     forward(stripeHeight) : REM color = ffffff
3030     left(90)
3040     penUp()
3050   NEXT k
3060 END SUB
