...
Public Function readNumbers(fileName As String, numbers As integer(50), maxNumbers As integer) As integer
  ...
  Try
    Do While not fileEOF(fileNo) and nNumbers < maxNumbers
      ...
      nNumbers <- nNumbers + 1
    Loop
  Catch ex1d8d30f7 As Exception
    Dim error As String = ex1d8d30f7.ToString()
    Throw 
  Finally
    fileClose(fileNo)
  End Try
...
Rem   fileClose(fileNo) 
  ...
  Catch ex5bb21b69 As Exception
    Dim failure As String = ex5bb21b69.ToString()
    PRINT failure
...
If fileNo > 0 Then
  ...
  Catch ex4abdb505 As Exception
    Dim failure As String = ex4abdb505.ToString()
    PRINT failure
  End Try
  If nObtained > 0 Then
    min <- numberArray[0]
