...
Public Function readNumbers(fileName As String, numbers As integer(50), maxNumbers As integer) As integer
  ...
  Try
    Do While not fileEOF(fileNo) and nNumbers < maxNumbers
      ...
      nNumbers <- nNumbers + 1
    Loop
  Catch ex757942a1 As Exception
    Dim error As String = ex757942a1.ToString()
    Throw 
  Finally
    fileClose(fileNo)
  End Try
...
Rem   fileClose(fileNo) 
  ...
  Catch ex42eca56e As Exception
    Dim failure As String = ex42eca56e.ToString()
    PRINT failure
...
If fileNo > 0 Then
  ...
  Catch ex569cfc36 As Exception
    Dim failure As String = ex569cfc36.ToString()
    PRINT failure
  End Try
  If nObtained > 0 Then
    min <- numberArray[0]
